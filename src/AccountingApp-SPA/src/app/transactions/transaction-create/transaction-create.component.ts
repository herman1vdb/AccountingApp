...
export class TransactionCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription = this.transactionService.newTransaction.subscribe(
      transaction => {
  ...
  controlSelected(event) {
    ...
    this.selectedControlAccount = this.controlAccounts.find(
      a => a.id.toString() === id
    );
    this.transactionService.selectedControlAccount.next(
      this.selectedControlAccount
    );
  ...
  assignAccountId() {
    if (this.transactionService.selectedTab.value === "receipts") {
      ...
      this.alertify.error(
        "Could not save, no tab selected, please refresh page"
      );
      return false;
    }
